...
@Component({
  ...
  styles: [`
    @media print {
      ...
      .print-full-width { grid-column: 1 / -1 !important; width: 100% !important; }
      .shadow { box-shadow: none !important; border: 1px solid #eee !important; }
    }
  `]
})
export class LignesVenteComponent implements OnInit {
  ...
  selectedLigne: any = { id_ligne: null, id_parfum: null, quantite: 1, prix_unitaire_facture: 0 };
  loading = false;
  saving = false;
  ...
  onParfumChange() {
    ...
    if (p) {
      this.selectedLigne.prix_unitaire_facture = p.prix_vente;
    }
  }

  resetForm() {
    this.selectedLigne = { id_ligne: null, id_parfum: null, quantite: 1, prix_unitaire_facture: 0 };
  }

  editLigne(ligne: any) {
    this.selectedLigne = { 
      ...ligne,
      // Gestion robuste : mapping vers la propriété du formulaire
      prix_unitaire_facture: ligne.prix_unitaire_facture !== undefined ? ligne.prix_unitaire_facture : (ligne.prix_unitaire || 0)
    };
  }
  ...
  async saveLigne() {
    ...
    try {
      await this.rpcService.upsertLigneVente(
        ...
        this.selectedLigne.prix_unitaire_facture || 0
      );
  ...
  getTotal() {
    return this.lignes.reduce((acc, l) => acc + (l.quantite * (l.prix_unitaire_facture || 0)), 0);
  }
}